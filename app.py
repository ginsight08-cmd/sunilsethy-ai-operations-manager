...
.stApp {{
    background:
        radial-gradient(circle at 90% 0%, rgba(0,174,239,.10), transparent 28%),
        radial-gradient(circle at 5% 25%, rgba(7,87,184,.055), transparent 25%),
...
.gi-brand span {{ color: var(--gi-blue); }}
...
.hero {{
    ...
    background:
        linear-gradient(135deg, rgba(7,87,184,.10), rgba(0,174,239,.055) 55%, rgba(255,157,0,.065));
    box-shadow: 0 16px 45px rgba(7,87,184,.075);
...
.auth-heading {{ text-align:center; color:var(--gi-navy); font-size:clamp(1.65rem, 3vw, 2.25rem); font-weight:800; line-height:1.15; margin:.2rem 0 .35rem; letter-spacing:-.8px; }}
.auth-description {{ text-align:center; color:#475467; font-size:1.03rem; margin-bottom:1rem; }}
.auth-note {{ text-align:center; color:var(--gi-muted); font-size:.8rem; margin-top:.8rem; }}
.auth-card {{ background:rgba(255,255,255,.94); border:1px solid var(--gi-border); border-radius:24px; padding:.65rem .8rem 1rem; box-shadow:0 20px 60px rgba(7,26,61,.09); backdrop-filter: blur(10px); }}
...
.small-muted {{ color:var(--gi-muted); font-size:.86rem; }}
...
section[data-testid="stFileUploaderDropzone"] {{
    background: linear-gradient(135deg, rgba(7,87,184,.035), rgba(0,174,239,.035));
...
.gi-footer {{ text-align:center; color:var(--gi-muted); font-size:.8rem; padding:1.5rem 0; }}

@media (max-width: 768px) {{
    .main .block-container {{ padding-left: .8rem; padding-right: .8rem; }}
    .auth-card {{ padding: .35rem; }}
...
if not st.session_state.authenticated:
    ...
    _, brand_col, _ = st.columns([1, 2, 1])
    with brand_col:
        if LOGO_PATH.exists():
            st.image(str(LOGO_PATH), width=300) 
        else:
            st.markdown(
                '<div class="gi-brand" style="text-align:center;">Generative <span>Insight</span></div>'
                '<div class="gi-tagline" style="text-align:center;">Insights today. Intelligence tomorrow.</div>',
                unsafe_allow_html=True,
            )

    st.markdown(
        f'<div style="text-align:center; margin-top:-4px; margin-bottom:20px;">'
        f'<span style="color:#667085; font-size:0.88rem;">AI / ML &nbsp; | &nbsp; Annotation &nbsp; | &nbsp; Web & App Development</span>'
        f'<span style="color:#B8C5D8;">&nbsp; · &nbsp;</span>'
        f'<a href="{WEBSITE_URL}" target="_blank" style="font-weight:700; text-decoration:none;">Visit Website ↗</a>'
        f'</div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        '<div class="hero">'
        '<div class="section-kicker" style="display:block; width:max-content; margin:0 auto .55rem;">AI OPERATIONS INTELLIGENCE</div>'
        '<div class="auth-heading">AI-powered operational intelligence</div>'
        '<div class="auth-description">Turn operational data into management decisions.</div>'
        '<p style="text-align:center; color:#475467; margin-bottom:0;">'
        'Create your account, upload Excel/CSV operational data, identify KPI risks, '
        'investigate team and employee performance, ask the AI Operations Copilot '
    ...
    with auth_center:
        ...
        with signup_tab:
            ...
            with st.form("signup_form", clear_on_submit=False):
                ...
                signup_password = st.text_input("Password", type="password", help="Use a strong password. Supabase enforces the configured password policy.")
                ...
                if not signup_name.strip():
                    ...
                    try:
                        with st.spinner("Creating your account..."):
                            signup_response = sign_up_user(signup_name, signup_company, signup_email, signup_password)
                        ...
                        if signup_user is not None and signup_session is not None:
                            ...
                            st.success("✅ Account created. Please check your email and click the verification link before signing in.")
                        else:
                            st.info("If the email is valid, check your inbox for the verification email.")
                    except Exception as e:
                        st.error("❌ Could not create account: " + friendly_auth_error(e))

            st.caption("By creating an account, you agree to use the platform responsibly and validate AI recommendations before taking material business action.")
        ...
        with login_tab:
            ...
            with st.form("login_form"):
                login_email = st.text_input("Email", placeholder="name@company.com", key="login_email")
            ...
            st.markdown('<div class="auth-note">🔒 Your account is secured through Supabase Authentication.</div>', unsafe_allow_html=True)
            st.info("If email confirmation is enabled in Supabase, verify your email before signing in.")

        with pricing_tab:
            show_pricing(section_id="auth")
...
st.markdown('<div class="section-kicker">EXECUTIVE OPERATIONS INTELLIGENCE</div>', unsafe_allow_html=True)
