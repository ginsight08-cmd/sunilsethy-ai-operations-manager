...
<style>
    .stApp {{
        background:
            radial-gradient(circle at 85% 0%, rgba(0,174,239,0.08), transparent 30%),
            linear-gradient(180deg, #FFFFFF 0%, #F7FAFF 100%);
    }}
    .main {{ padding-top: 1.5rem; }}
    .main-title {{
        font-size: 2.35rem; font-weight: 850; color: {BRAND_NAVY};
        margin-bottom: 0.15rem; letter-spacing: -1px;
    }}
    .brand-subtitle {{ color: #667085; font-size: 1.02rem; margin-bottom: 1rem; }}
    .gi-brand {{ font-size: 1.45rem; font-weight: 800; color: {BRAND_NAVY}; letter-spacing: -0.5px; }}
    .gi-brand span {{ color: {BRAND_BLUE}; }}
    .gi-tagline {{ color: #667085; font-size: 0.82rem; margin-top: 0.15rem; }}
    .hero {{
        padding: 1.45rem 1.6rem; border-radius: 20px; border: 1px solid #DCE8F8;
        background: linear-gradient(135deg, rgba(7,87,184,0.08), rgba(0,174,239,0.04), rgba(255,157,0,0.05));
        box-shadow: 0 8px 30px rgba(7,87,184,0.06); margin-bottom: 1.2rem;
    }}
    .hero h3 {{ color: {BRAND_NAVY} !important; margin: 0 0 4px 0; }}
    .hero p {{ color: #667085 !important; margin: 0; }}
    .small-muted {{ color: #667085; font-size: .88rem; }}
    div[data-testid="stMetric"] {{
        background: #FFFFFF; border: 1px solid #E0E8F5; border-radius: 16px;
        padding: 0.8rem; box-shadow: 0 4px 15px rgba(7,87,184,0.05);
    }}
    div[data-testid="stMetricValue"] {{ color: {BRAND_NAVY}; font-weight: 800; }}
    div[data-testid="stMetricLabel"] {{ color: #475467; }}

    .plan-card {{
        padding: 1.35rem; border-radius: 18px; border: 1px solid #DDE7F5;
        background: #FFFFFF; min-height: 300px;
        box-shadow: 0 8px 24px rgba(7,87,184,0.06);
        color: {BRAND_NAVY};
        display: flex; flex-direction: column; gap: 0.35rem;
    }}
    .plan-card:hover {{ border-color: {BRAND_CYAN}; box-shadow: 0 10px 30px rgba(0,174,239,0.12); }}
    .plan-card * {{ color: {BRAND_NAVY} !important; }}
    .plan-icon {{
    ...
    .plan-name {{ font-size: 1.15rem; font-weight: 800; margin: 0; }}
    .plan-price {{ font-size: 1.55rem; font-weight: 850; margin: 0 0 0.3rem 0; }}
    .plan-features {{ list-style: none; padding: 0; margin: 0; }}
    .plan-features li {{
        font-size: 0.92rem; padding: 3px 0; color: {BRAND_NAVY} !important;
    }}
    .plan-features li b {{ color: {BRAND_CYAN} !important; margin-right: 6px; }}
    ...
    .stButton > button {{ border-radius: 10px; font-weight: 700; border: 1px solid #C9D8EE; }}
    .stButton > button[kind="primary"] {{
        background: linear-gradient(90deg, {BRAND_BLUE}, {BRAND_CYAN}); color: white; border: none;
    }}
    .stButton > button[kind="primary"]:hover {{
        background: linear-gradient(90deg, #064A9D, #009BD5); color: white;
    }}

    button[data-baseweb="tab"] {{ font-weight: 700; }}
    button[data-baseweb="tab"][aria-selected="true"] {{ color: {BRAND_BLUE}; }}

    a {{ color: {BRAND_BLUE}; }}

    .gi-footer {{ text-align: center; color: #667085; font-size: 0.82rem; padding: 1.5rem 0; }}
</style>
""",
...
def show_brand_header(compact=False):
    ...
    if LOGO_PATH.exists():
        st.image(str(LOGO_PATH), width=230 if compact else 420)
    else:
        st.markdown(
    ...
    st.markdown(
        f"""
        <div style="margin-top:-8px; margin-bottom:18px; color:#667085; font-size:0.85rem;">
...
def get_plan_config(plan):
    configs = {
        "Free": {
            "max_mb": 5, "copilot": True, "pdf": True, "email": False,
            "automation": False, "price": "₹0",
        },
        "Professional": {
            "max_mb": 25, "copilot": True, "pdf": True, "email": True,
            "automation": True, "price": "₹1,999/mo",
        },
        "Business": {
            "max_mb": 100, "copilot": True, "pdf": True, "email": True,
            "automation": True, "price": "Custom",
        },
    }
...
def build_copilot_context(
    company_name, report_name, result, productivity_target, quality_target,
    sla_target, aht_target, risk_level, summary_points,
):
...
def create_pdf_report(
    company_name, report_name, result, risk_level, summary_points, recommendation,
):
    try:
    ...
    title_style = ParagraphStyle(
        "GI_Title", parent=styles["Title"], fontSize=20, leading=24, spaceAfter=8,
    )
    heading_style = ParagraphStyle(
        "GI_Heading", parent=styles["Heading2"], fontSize=13, leading=16,
        spaceBefore=10, spaceAfter=6,
    )
    body_style = ParagraphStyle(
        "GI_Body", parent=styles["BodyText"], fontSize=9, leading=12,
    )
    ...
    kpi_rows = [
        ["KPI", "Actual", "Target"],
        ["Productivity", f'{float(overall["productivity"]):.2f}%',
         f'{result.get("_targets", {}).get("productivity", "")}%'],
        ["Quality", f'{float(overall["quality"]):.2f}%',
         f'{result.get("_targets", {}).get("quality", "")}%'],
        ["SLA", f'{float(overall["sla"]):.2f}%',
         f'{result.get("_targets", {}).get("sla", "")}%'],
        ["Average AHT", f'{float(overall["aht"]):.2f}',
...
def send_email_report(
    recipient, subject, body, pdf_bytes=None, pdf_filename="operations_report.pdf",
):
...
# ============================================================
# PRICING PLAN ICONS (self-contained inline SVGs, brand-colored)
# ============================================================
...
PLAN_ICONS = {
    ...
    "Professional": (
        BRAND_BLUE,
        """<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M12 2.5 14.9 8.6 21.5 9.6 16.8 14.2 17.9 20.8 12 17.6 6.1 20.8 7.2 14.2 2.5 9.6 9.1 8.6 12 2.5Z" fill="white"/>
        </svg>""",
    ),
    "Business": (
        BRAND_NAVY,
        """<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M4 21V8l6-4v17H4Z" fill="white" fill-opacity="0.9"/>
            <path d="M13 21V3l7 3v15h-7Z" fill="white"/>
            <rect x="6.5" y="10" width="1.6" height="1.6" fill="#0757B8"/>
            <rect x="6.5" y="14" width="1.6" height="1.6" fill="#0757B8"/>
            <rect x="15.5" y="8" width="1.6" height="1.6" fill="#071A3D"/>
            <rect x="15.5" y="12" width="1.6" height="1.6" fill="#071A3D"/>
            <rect x="15.5" y="16" width="1.6" height="1.6" fill="#071A3D"/>
        </svg>""",
    ),
}
...
def show_pricing(section_id="default"):
    ...
    for col, name, price, features, button in plans:
        ...
        with col:
            ...
            icon_color, icon_svg = PLAN_ICONS.get(name, PLAN_ICONS["Free"])

            features_html = "".join(
                f'<li><b>✓</b>{feature}</li>' for feature in features
            )

            card_html = f"""
            <div class="plan-card">
                <div class="plan-icon" style="background:{icon_color};">
                    {icon_svg}
                </div>
            ...
            if checkout_url:
                ...
                st.button(
                    button, use_container_width=True, disabled=True,
    ...
    with signup_tab:
        ...
        with st.form("signup_form", clear_on_submit=False):
            ...
            signup_company = st.text_input(
                "Company / Organization", placeholder="e.g. ABC Technologies"
            )
                ...
                try:
                    with st.spinner("Creating your account..."):
                        signup_response = sign_up_user(
                            signup_name, signup_company, signup_email, signup_password,
                        )
            ...
            if not login_email.strip() or not login_password:
                ...
                try:
                    with st.spinner("Signing you in..."):
                        login_response = sign_in_user(login_email, login_password)
...
with st.sidebar:
    ...
    productivity_target = st.number_input(
        "Productivity target %", min_value=1, max_value=200, value=90,
    )
    quality_target = st.number_input(
        "Quality target %", min_value=1, max_value=100, value=95,
    )
    sla_target = st.number_input(
        "SLA target %", min_value=1, max_value=100, value=97,
    )
    aht_target = st.number_input(
        "AHT target", min_value=1, max_value=1000, value=50,
    )

    st.divider()

    if st.button("💳 View Plans", use_container_width=True, key="sidebar_view_plans"):
        st.session_state.show_plans = True

    if st.button("🔄 Reset Analysis", use_container_width=True, key="sidebar_reset_analysis"):
        clear_analysis()
        st.rerun()
...
with col3:
    report_name = st.text_input("Report Name", value="Daily Operations Report")
...
def read_uploaded_data(uploaded_file):
    ...
    if suffix == ".xlsx":
        ...
        for sheet_name in candidates:
            try:
                data = pd.read_excel(io.BytesIO(raw), sheet_name=sheet_name, engine="openpyxl")
...
required_columns = [
    "Employee_ID", "Employee_Name", "Team", "Target", "Production",
    "AHT_Actual", "Quality_%", "SLA_%",
]
...
try:
    result = analyze_data(
        df, productivity_target=productivity_target, quality_target=quality_target,
        sla_target=sla_target, aht_target=aht_target,
    )
except Exception as e:
    ...
    for col in ["Priority", "priority", "Priority_Level", "priority_level"]:
        if col in actions_df.columns:
            high_priority_count = len(
                actions_df[
                    actions_df[col].astype(str).str.lower().isin(["high", "critical"])
                ]
            )
            break
...
with r2:
    st.metric("KPI Breaches", breaches, delta=f"{4 - breaches} on target")
...
with k1:
    st.metric("Productivity", f"{productivity:.2f}%", delta=f"{productivity_gap:+.2f}% vs target")

with k2:
    st.metric("Quality", f"{quality:.2f}%", delta=f"{quality_gap:+.2f}% vs target")

with k3:
    st.metric("SLA", f"{sla:.2f}%", delta=f"{sla_gap:+.2f}% vs target")

with k4:
    st.metric("Average AHT", f"{aht:.2f}", delta=f"{aht_gap:+.2f} vs target")
...
tabs = st.tabs(
    [
        "📊 Executive Dashboard", "🚨 AI Insights", "👥 Employee Risk",
        "✅ Action Center", "🤖 Management Copilot", "📄 Reports", "💳 Billing",
    ]
)
    ...
    with right:
        ...
        if isinstance(employees, pd.DataFrame) and not employees.empty:
            ...
            if "Risk_Score" in employees.columns:
                st.metric("Highest employee risk score", f"{employees['Risk_Score'].max():.2f}")
...
with tabs[2]:
    ...
    if isinstance(employee_data, pd.DataFrame) and not employee_data.empty:
        ...
        sort_cols = [
            c for c in ["Risk_Score", "Avg_Productivity"] if c in employee_data.columns
        ]
...
with tabs[4]:
    ...
    question = st.text_input(
        ...
        placeholder="Which team has the quality drop and what action should be taken?",
        key="copilot_question",
    )

    ask_copilot = st.button(
        "🚀 Ask Management Copilot", type="primary",
        use_container_width=True, key="ask_management_copilot",
    )
    ...
    if ask_copilot:
        ...
        if not question.strip():
            ...
            context = build_copilot_context(
                company_name, report_name, result, productivity_target,
            ...
            try:
                ...
                if copilot_response.status_code < 300:
                    ...
                    st.session_state.copilot_answer = None
                    st.error(f"❌ Copilot workflow failed: HTTP {copilot_response.status_code}")
...
with tabs[5]:
    ...
    if not plan_config["pdf"]:
        ...
        if st.button(
            "📄 Generate Executive PDF", type="primary",
            use_container_width=True, key="generate_executive_pdf",
        ):
            try:
                with st.spinner("Generating management report..."):
                    pdf = create_pdf_report(
                        ...
                        report_result, risk_level, summary_points, recommendation,
                    )
        ...
        if st.session_state.report_pdf:
            ...
            st.download_button(
                "⬇️ Download Executive PDF", data=st.session_state.report_pdf,
                file_name=filename, mime="application/pdf",
                use_container_width=True, key="download_executive_pdf",
            )

            if st.session_state.report_generated_at:
                st.caption(
                    "Generated "
                    + st.session_state.report_generated_at.strftime("%d %b %Y, %H:%M")
                )
        ...
        if not plan_config["email"]:
            st.info("Email delivery is available on Professional and Business plans.")
        else:
            recipient = st.text_input(
                "Recipient email", value=manager_email, key="report_recipient",
            )

            if st.button("📨 Email PDF Report", use_container_width=True, key="email_pdf_report"):
        ...
        with e1:
            ...
            if isinstance(team_df, pd.DataFrame):
                st.download_button(
                    ...
                    "team_analysis.csv", "text/csv",
                    use_container_width=True, key="download_team_analysis",
                )
        ...
        with e2:
            if isinstance(actions_df, pd.DataFrame):
                st.download_button(
                    ...
                    use_container_width=True, key="download_action_plan",
                )
